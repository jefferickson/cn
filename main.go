...
func main() {
	...
	value, err := findNumber(fileCache[0], args[0], ",")
	if err != nil {
		os.Exit(1)
	}

	if printData {
		for n, row := range fileCache {
			if n == 0 {
				continue
			}

			if cell := extractCol(row, value-1, ","); cell != "" {
				fmt.Println(cell)
			}
		}
	} else {
...
func findNumber(rowData string, label string, delim string) (int, error) {
